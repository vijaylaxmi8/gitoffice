  ...
  ${import_picocolors.default.red(`${import_picocolors.default.bold("LARAVEL")} ${laravelVersion()}`)}  ${import_picocolors.default.dim("plugin")} ${import_picocolors.default.bold(`v${pluginVersion()}`)}`);
      ...
      return () => server.middlewares.use((req, res, next) => {
        if (req.url === "/index.html") {
          res.statusCode = 404;
          res.end(
            import_fs.default.readFileSync(import_path.default.join(__dirname, "dev-server-index.html")).toString().replace(/{{ APP_URL }}/g, appUrl)
          );
        }
        next();
      });
...
function pluginVersion() {
  try {
    return JSON.parse(import_fs.default.readFileSync(import_path.default.join(__dirname, "../package.json")).toString())?.version;
  } catch {
    return "";
  }
}
